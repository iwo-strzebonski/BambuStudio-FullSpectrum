...
void MixedFilamentManager::auto_generate(const std::vector<std::string> &filament_colours,
                                         const std::vector<double> &nozzle_diameters_per_filament)
{
    ...
    // Helper: returns true when filaments i and j (0-based) may be paired.
    // When nozzle diameter info is available, only same-diameter filaments
    // are eligible (mixing 0.4mm and 0.6mm nozzles would produce
    // inconsistent layer widths/heights). Diameters are compared with a
    // tolerance of 0.01mm (e.g. 0.399 ≈ 0.401).
    const bool diameter_aware = !nozzle_diameters_per_filament.empty()
                                && nozzle_diameters_per_filament.size() >= n;
    auto can_pair = [&](size_t i, size_t j) -> bool {
        if (!diameter_aware)
            return true;
        return std::abs(nozzle_diameters_per_filament[i] - nozzle_diameters_per_filament[j]) < 0.01;
    };

    // Generate pairwise combinations — respecting nozzle-diameter groups.
