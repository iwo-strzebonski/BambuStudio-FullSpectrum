...
public:
    ...
    // filament colours.  Generates pairwise combinations.
    // If nozzle_diameters_per_filament is non-empty, only filaments that
    // share the same nozzle diameter are paired (nozzle-diameter-aware mode
    // for toolchangers like H2C with potentially mixed nozzle sizes).
